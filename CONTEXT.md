...
> Canonical terms used across Thunder specs, code, and discussions.
> If a term isn't here, it isn't agreed upon yet.

| Term | Definition |
|---|---|
| **Thunder** | Unified Headless Download Daemon. The Python/FastAPI backend that orchestrates downloads across engines. |
...
| **Ghost Overlay** | The fixed-position, pointer-events-none host element (`#thunder-host`) that escapes host page stacking contexts. Internal elements use `pointer-events: auto`. |
